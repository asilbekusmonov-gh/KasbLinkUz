...
class Conversation(Model):
    client = ForeignKey("apps.User", CASCADE, related_name="client_conversations")
...
class Message(CreatedAt):
    ...
    message_type = CharField(max_length=20, choices=MessageType.choices, default=MessageType.TEXT)

    message = TextField(blank=True, null=True)

    image = ImageField(upload_to="messages/images/", blank=True, null=True)

    video = FileField(upload_to="messages/videos/", blank=True, null=True)
